...
export function err<const T extends string, const I extends Issues>(error: ErrVariant<T, I>): Err<T, I> {
  return {
    error,
    result: null,
  };
...
export function okMap<
  const OldResult extends NewResult,
  const NewResult,
  const E extends string,
  I extends Issues = Issues,
>(result: Result<OldResult, E, I>, okCb: (arg: OldResult) => NewResult): Result<NewResult, E, I> {
  if (result.error) return result;
